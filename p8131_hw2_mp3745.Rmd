...
The logit model has the highest estimate for β, with the probit model having the lowest. All estimates of β are positive, which means the risk of dying increases with dose. For the logit model, the interpretation is that the log odds ratio per 1 unit change in dose is 1.1619. All three confidence intervals do not contain zero, which further suggests the risk of dying increases with dose. The probit model had the smallest deviance and the c-log-log model had the highest deviance. This suggests that the logit and probit models may fit the data better. All three models provide similar estimates for the probability of dying given the dose is 0.01, but the c-log-log model has the highest estimate at 0.13.
...
c(prob2_x0fit, sqrt(prob2_varx0))

# 95% CI 
(prob2_x0fit + c(qnorm(0.025), -qnorm(0.025))*sqrt(prob2_varx0))
```
